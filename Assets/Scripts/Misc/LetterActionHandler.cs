...
public class LetterActionHandler : MonoBehaviour
{
    ...
    public GameObject pacman_alive; // The object that will be hidden
    public GameObject pacman_dead; // The object that will be shown
    ...
    private IEnumerator FallAndReturn(GameObject letterObject)
    {
        ...
        while (Vector3.Distance(letterRect.localPosition, dasherLocalPosition) > distanceThreshold)
        {
            ...
            letterRect.localPosition = Vector3.MoveTowards(
                letterRect.localPosition,
                dasherLocalPosition,
    ...
    private void TogglePacmanWithDelay()
    {
            if (pacman_alive != null && pacman_dead != null)
            {
                // Hide pacman_alive and show pacman_dead
                pacman_alive.SetActive(false);
                pacman_dead.SetActive(true);

                // Optionally: Disable components on pacman_alive and enable components on pacman_dead
                SetComponentsEnabled(pacman_alive, false);
                SetComponentsEnabled(pacman_dead, true);

                // Start the coroutine to revert the change after 1.5 seconds
                StartCoroutine(RevertPacmanAfterDelay(1.375f));
            }
    }

  // Coroutine to revert the active state and components' enabled state after a delay
    private IEnumerator RevertPacmanAfterDelay(float delay)
    {
        // Wait for the specified delay
        yield return new WaitForSeconds(delay);

        // Revert: Show pacman_alive and hide pacman_dead
        pacman_alive.SetActive(true);
        pacman_dead.SetActive(false);

        // Optionally: Re-enable components on pacman_alive and disable components on pacman_dead
        SetComponentsEnabled(pacman_alive, true);
        SetComponentsEnabled(pacman_dead, false);
    }

    // Method to enable or disable all components on a GameObject (if needed)
    private void SetComponentsEnabled(GameObject obj, bool enabled)
    {
        // Get all components on the GameObject
        Component[] components = obj.GetComponents<Component>();

        // Loop through all components and enable/disable them
        foreach (Component component in components)
        {
            // We only toggle components that have the "enabled" property
            if (component is Behaviour behaviourComponent)
            {
                behaviourComponent.enabled = enabled;
            }
        }
    }
